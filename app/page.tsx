...
import { PortfolioOrbit } from "@/components/portfolio-orbit";
...
export default async function Home() {
  ...
  return <PublicShell>
    <section className="hero hero-composed wrap">
      <div className="hero-intro"><p className="eyebrow">{config?.heroTagline ?? "Hi, I’m Alya."}</p>
      <h1>{headline}</h1>
      <p className="hero-copy">{config?.heroSubheadline ?? "Business analysis, systems, software, and everything in between."}</p>
      <div className="hero-actions"><Link className="button" href={config?.heroCta1Url ?? "/work"}>{config?.heroCta1Text ?? "Explore my work"}</Link><Link className="text-link" href="#approach">See my approach <span>↓</span></Link></div></div>
      <div className="hero-visual"><div className="hero-photo-frame"><Image src={person?.photoUrl ?? "/assets/photos/alya-professional-1.jpg"} alt="Alya Nisrina" fill priority sizes="(max-width: 700px) 82vw, 36vw" /></div><PortfolioOrbit /><p className="hero-visual-caption">Business × Technology<br /><span>Evidence-led practice</span></p></div>
      <p className="hero-note">Problem → Understand → Analyze → Design → Build → Validate → Improve</p>
    </section>
    <Reveal><section id="approach" className="philosophy wrap"><p className="eyebrow">How I think</p><blockquote>“{person?.philosophy ?? "Done is not the goal. Solving the right problem is."}”</blockquote><p>I start by understanding what needs to be solved, then connect people, process, and technology into something useful.</p><ol className="approach-steps">{["Understand", "Analyze", "Design", "Build", "Validate", "Improve"].map((step, index) => <li key={step}><span>0{index + 1}</span>{step}</li>)}</ol></section></Reveal>
