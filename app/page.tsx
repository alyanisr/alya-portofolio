...
export default async function Home() {
  const { config, person, projects, experiences, achievements, skillCategories, learning, lenses } = await getHomepage();
  ...
  return <PublicShell>
    ...
    {lenses.length > 0 && <Reveal><section className="lens-band"><div className="wrap"><SectionHeading eyebrow="One identity, multiple lenses" title="How do you want to know me?"><p>The same evidence, intentionally prioritized for the role you are exploring.</p></SectionHeading><div className="lens-grid">{lenses.map((lens) => <Link href={`/recruiter/${lens.slug}`} key={lens.id}><span>Recruiter lens</span><h3>{lens.name}</h3><p>{lens.intro}</p><b>See this perspective →</b></Link>)}</div></div></section></Reveal>}
